...
function setTierList(imported) {
	...
	poolItems.forEach(item => {
		...
		if (!hasItem) {
			...
			element.classList.add('tier-item', 'draggable');
			element.style.backgroundImage = "url('" + dirTierItems + item + '.' + tierItemExt + "')";
...
function saveTierList() {
	...
	localStorage.setItem(storageKey, JSON.stringify(tierList));
...
function createTierElement(tier, targetTierIndex) {
	...
	tier.items.forEach(item => {
		...
		element.style.backgroundImage = "url('" + dirTierItems + item.name + '.' + tierItemExt + "')";
